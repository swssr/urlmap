    ...
    return (
      <div className="mt-6 space-y-6">
        ...
        <div className="bg-gray-50 p-3 rounded font-mono text-sm overflow-x-auto border border-gray-200">
          {parsedUrl.protocol && (
            <span
              className={`${colorMap.protocol.bg} ${colorMap.protocol.text} px-1 rounded`}
            >
              {parsedUrl.protocol}//
            </span>
          )}
          {parsedUrl.username && (
            <span
              className={`${colorMap.username.bg} ${colorMap.username.text} px-1 rounded`}
            >
              {parsedUrl.username}
            </span>
          )}
          {parsedUrl.password && (
            <>
              <span>:</span>
              <span
                className={`${colorMap.password.bg} ${colorMap.password.text} px-1 rounded`}
              >
                {parsedUrl.password}
              </span>
            </>
          ...
          {parsedUrl.hostname && (
            <span
              className={`${colorMap.hostname.bg} ${colorMap.hostname.text} px-1 rounded`}
            >
              {parsedUrl.hostname}
            </span>
          )}
          {parsedUrl.port && (
            <>
              <span>:</span>
              <span
                className={`${colorMap.port.bg} ${colorMap.port.text} px-1 rounded`}
              >
                {parsedUrl.port}
              </span>
            </>
          ...
          {parsedUrl.pathname && (
            <span
              className={`${colorMap.pathname.bg} ${colorMap.pathname.text} px-1 rounded`}
            >
              {parsedUrl.pathname}
            </span>
          )}
          {parsedUrl.search && (
            <span
              className={`${colorMap.search.bg} ${colorMap.search.text} px-1 rounded`}
            >
              {parsedUrl.search}
            </span>
          )}
          {parsedUrl.hash && (
            <span
              className={`${colorMap.hash.bg} ${colorMap.hash.text} px-1 rounded`}
            >
              {parsedUrl.hash}
            </span>
          )}
            ...
            <div className="space-y-2">
              {group.items.map((item) => (
                <div key={item.key} className="flex">
                  <div className="w-60 text-nowrap text-gray-500 text-sm">
                    {item.label}
                  </div>
                  {colorMap[item.key] && item.value !== "-" ? (
                    <div
                      className={`font-mono text-sm ${colorMap[item.key].bg} ${colorMap[item.key].text} px-2 py-0.5 rounded overflow-auto`}
                    >
                      {item.value}
                    </div>
                  ) : (
        ...
        {parsedUrl.searchParams &&
          Object.keys(parsedUrl.searchParams).length > 0 && (
            <div className="border-t border-gray-200 pt-4">
              ...
              <div
                className={`rounded ${colorMap.search.bg} border ${colorMap.search.border}`}
              >
                {Object.entries(parsedUrl.searchParams).map(([key, value]) => (
                  <div
                    key={key}
                    className="flex py-2 px-2 border-b border-gray-100 last:border-b-0"
                  >
                    <div className="w-60 text-gray-700 text-sm">{key}</div>
